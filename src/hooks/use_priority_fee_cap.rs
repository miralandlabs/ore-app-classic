...
pub fn use_priority_fee_cap() -> Signal<PriorityFeeCap> {
    ...
    use_effect(move || priority_fee_cap_persistent.set(*priority_fee_cap.read()));
    priority_fee_cap
}

pub fn use_priority_fee_cap_provider() {
    let priority_fee_cap = use_persistent(KEY, || PriorityFeeCap(PRIORITY_FEE_CAP)).get();
    use_context_provider(|| Signal::new(priority_fee_cap));
}
